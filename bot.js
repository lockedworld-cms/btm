...
const setupCMD = "l!murat" //İstediğiniz Komut Yapabilirsiniz örn : !kayıtol
...
const botToken = "NTgxNTk2NDMyNzgwMzYxNzUw.XOuh8A.qWwdrIz1EBcuKBN7H4GkSSsVToA";  //Buraya botunuzun tokenini koyunuz
...
client.on('message', msg => {
  if (msg.content.toLowerCase() === 'l!temizle') {
...
///panel
client.on("message", m => {
client.channels.get("609413357098762263").setName(`Sunucu Sayısı: ${client.guilds.size}`)
client.channels.get("609413414984482818").setName(`Kullanıcı Sayısı: ${client.users.size}`)
client.channels.get("609413456260628481").setName(`Kanal Sayısı: ${client.channels.size}`)
});
///panrl
...
.addField("Sunucudaki Kişi Sayısı:", guild.memberCount)
   ...
   client.channels.get('610123942732234802').send(rrrsembed);
...
.addField("Sunucu sahibi", guild.owner)
.addField("Sunucu Sahibi'nin ID'si", guild.ownerID)
.addField("Sunucu Davet Linki", guild.invite)
.addField("Sunucunun Kurulu Olduğu Bölge:", guild.region)
.addField("Sunucudaki Kişi Sayısı:", guild.memberCount)

   client.channels.get('610124034050490385').send(rrrsembed);
